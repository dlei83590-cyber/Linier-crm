 ...
 * - **数量 ceiling**：`quantity <= qualifiedQty`，且**累计入库（所有非 CANCELLED 单行）不得超过该 Inspection 可入库余额**；
...
/** 某 Inspection 已被占用（所有非 CANCELLED WarehouseReceipt 行）的累计入库数量（可入库余额 = qualifiedQty - usedQty） */
  ...
  const agg = await tx.warehouseReceiptLine.aggregate({
    where: {
      ...
      ...(excludeWarehouseReceiptId
        ...
        : {}),
      warehouseReceipt: { deletedAt: null, status: { not: 'CANCELLED' } },
    },
    _sum: { quantity: true },
  });
