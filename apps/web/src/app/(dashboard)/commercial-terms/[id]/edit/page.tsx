...
/** Commercial Terms — 编辑商业条款（Pending Pages Completion Gate — Batch 1；CAS version） */
...
interface CommercialTermDetail {
  id: string;
  code: string;
  name: string;
...
function CommercialTermEditForm() {
  ...
  const load = () => {
    setLoading(true);
    setLoadError(null);
    apiFetch<CommercialTermDetail>(`/api/commercial-terms/${id}`)
      .then((body) => {
        const d = body.data;
        setCode(d.code);
        setName(d.name);
  ...
  const handleSave = () => {
    ...
    apiFetch<{ id: string }>(`/api/commercial-terms/${id}`, {
      method: "PATCH",
  ...
  if (loading) {
    return (
      <EntityFormWorkspace title="编辑商业条款" backHref="/commercial-terms" mode="edit" submitting={false} onSave={handleSave} onCancel={() => router.push("/commercial-terms")}>
  ...
  if (loadError) {
    return (
      <EntityFormWorkspace title="编辑商业条款" backHref="/commercial-terms" mode="edit" submitting={false} error={loadError} onSave={handleSave} onCancel={() => router.push("/commercial-terms")}>
...
export default function Page() {
  return (
    <PermissionGuard permission={actionPermission("commercial-term", "edit")}>
      <AppPage>
        <CommercialTermEditForm />
      </AppPage>
    </PermissionGuard>
  );
}