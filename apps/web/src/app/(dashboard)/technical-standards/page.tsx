...
function TechnicalStandardList() {
  ...
  const canCreate = can(state.user, actionPermission("technical-standard", "create"));
  const canEdit = can(state.user, actionPermission("technical-standard", "edit"));
  const canDelete = can(state.user, actionPermission("technical-standard", "delete"));
  ...
  const { items, total, page, pageSize, loading, error, setPage, setPageSize, refresh } =
    useListQuery<TechnicalStandardRow>("/api/technical-standards", filters, 20, { syncUrl: true });
