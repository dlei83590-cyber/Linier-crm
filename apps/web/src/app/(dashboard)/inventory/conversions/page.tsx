...
function ConversionList() {
  const [conversionNoInput, setConversionNoInput] = useState("");
  ...
  const applyFilter = () => {
    ...
    if (conversionNoInput.trim()) next.conversionNo = conversionNoInput.trim();
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white">
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 p-4">
        <h1 className="mr-4 text-lg font-semibold text-slate-800">库存转换</h1>
        <input
          value={conversionNoInput}
          onChange={(e) => setConversionNoInput(e.target.value)}
          onKeyDown={(e) => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50 text-left text-xs font-medium text-slate-500">
            <tr>
              <th className="px-4 py-3">转换单号</th>
              <th className="px-4 py-3">状态</th>
              <th className="px-4 py-3">物料</th>
              <th className="px-4 py-3">基准单位</th>
            ...
            {loading ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-800">{item.conversionNo}</td>
                  <td className="px-4 py-3">
                    <StatusBadge status={item.status} />
                  </td>
                  <td className="px-4 py-3 text-slate-700">
                    {item.item ? `${item.item.code ?? ""} ${item.item.name ?? ""}`.trim() : "—"}
                  </td>
                  <td className="px-4 py-3 text-slate-600">{item.baseUom?.symbol ?? "—"}</td>
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/inventory/conversions/${item.id}`} className="text-brand-600 hover:text-brand-700">
                      查看
                    </Link>
                  </td>
                </tr>
