...
function AdjustmentList() {
  ...
  const canCreate =
    ...
    state.user !== null &&
    can(state.user, actionPermission("inventory-adjustment", "create"));
  const canDelete = can(state.user, actionPermission("inventory-adjustment", "delete"));
  ...
  // 页面仪表盘 KPI：只读汇总（GET /api/inventory-adjustments/summary）；失败静默隐藏
  useEffect(() => {
    let cancelled = false;
    apiFetch<ModuleSummaryData>("/api/inventory-adjustments/summary")
      .then((b) => {
