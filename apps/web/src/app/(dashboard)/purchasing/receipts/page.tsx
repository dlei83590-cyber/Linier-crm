...
function ReceiptList() {
  ...
  const canCreate =
    ...
    state.user !== null &&
    hasPermission(state.user.roles as RoleCode[], "purchase-receipt:create");
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white">
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 p-4">
        <h1 className="mr-4 text-lg font-semibold text-slate-800">到货收货</h1>
        <input
          value={codeInput}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50 text-left text-xs font-medium text-slate-500">
            <tr>
              <th className="px-4 py-3">收货单号</th>
            ...
            {loading ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/purchasing/receipts/${item.id}`} className="text-brand-600 hover:text-brand-700">
                      查看
                    </Link>
                  </td>
                </tr>
