...
function ReceiptList() {
  ...
  // 页面仪表盘 KPI：只读汇总（GET /api/purchase-receipts/summary）；失败静默隐藏
  useEffect(() => {
    let cancelled = false;
    apiFetch<ModuleSummaryData>("/api/purchase-receipts/summary")
      .then((b) => {
  ...
  const { items, total, page, pageSize, loading, error, setPage, setPageSize, refresh } =
    useListQuery<ReceiptRow>("/api/purchase-receipts", filters, 20, { syncUrl: true });
