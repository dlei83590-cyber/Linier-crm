...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; sid: string }> }) {
  ...
  const txResult = await prisma.$transaction(async (tx) => {
    ...
    const existing = await tx.projectStakeholder.findFirst({ where: { id: sid, projectId: id, deletedAt: null } });
    if (!existing) return { error: failNotFound(ERROR_CODES.NOT_FOUND, "关系人不存在") };

  // A4-CAS：原子乐观锁（消除 read-check-update TOCTOU）
  const cas = await casUpdate(tx, "projectStakeholder", sid, version, {
    ...updates,
    role: updates.role as StakeholderRole | undefined,
    updatedById: user!.id,
  });
  if (cas.outcome === "NOT_FOUND") return { error: failNotFound(ERROR_CODES.NOT_FOUND, "关系人不存在") };
  if (cas.outcome === "CONFLICT") return { error: failConflict(ERROR_CODES.VERSION_CONFLICT, "版本冲突，请刷新后重试") };
  const updated = await tx.projectStakeholder.findFirst({ where: { id: sid, deletedAt: null } });
  if (!updated) return { error: failNotFound(ERROR_CODES.NOT_FOUND, "关系人不存在") };
