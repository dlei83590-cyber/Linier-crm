...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; bid: string }> }) {
  ...
  const txResult = await prisma.$transaction(async (tx) => {
    ...
    const cas = await casUpdate(tx, "projectBudget", bid, version, {
      ...updates,
      updatedById: user!.id,
    });
    if (cas.outcome === "NOT_FOUND") return { error: failNotFound(ERROR_CODES.NOT_FOUND, "预算不存在") };
    if (cas.outcome === "CONFLICT") return { error: failConflict(ERROR_CODES.VERSION_CONFLICT, "版本冲突，请刷新后重试") };
    const updated = await tx.projectBudget.findFirst({ where: { id: bid, deletedAt: null } });
    if (!updated) return { error: failNotFound(ERROR_CODES.NOT_FOUND, "预算不存在") };
