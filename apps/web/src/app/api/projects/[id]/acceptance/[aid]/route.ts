...
export async function GET(
  ...
  const item = await prisma.projectAcceptance.findFirst({
...
export async function PATCH(
  ...
  // L0 lifecycle integrity：mutation 与 Project header lock 同事务（B2-0 锁纪律）
  const txResult = await prisma.$transaction(async (tx) => {
    const gate = await assertProjectWritable(tx, id);
    if (!gate.ok) return { error: gate.response };

    const existing = await tx.projectAcceptance.findFirst({
      where: { id: aid, projectId: id, deletedAt: null },
    });
...
export async function DELETE(
  ...
  const txResult = await prisma.$transaction(async (tx) => {
    ...
    await tx.projectAcceptance.update({
      where: { id: aid },
