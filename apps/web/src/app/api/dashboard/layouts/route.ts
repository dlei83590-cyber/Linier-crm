...
export async function GET(request: NextRequest) {
  ...
  requestLog(request, user?.id, "dashboard-layout.list");
  ...
  const [total, items] = await Promise.all([
    ...
    prisma.dashboardLayout.findMany({ where, orderBy: [{ sort: "asc" }, { updatedAt: "desc" }], skip, take }),
  ]);
...
export async function POST(request: NextRequest) {
  ...
  if (denied) return denied;
  requestLog(request, user?.id, "dashboard-layout.create");
  ...
  await writeAuditLog({
    actorId: user?.id,
    action: "dashboard-layout.create",
