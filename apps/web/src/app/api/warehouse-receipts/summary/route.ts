...
/**
 * GET /api/warehouse-receipts/summary —— 模块页仪表盘 KPI
 *
...
export async function GET(request: NextRequest) {
  ...
  requestLog(request, user?.id, "warehouse-receipts.summary");

  const [total, byStatus] = await Promise.all([
    prisma.warehouseReceipt.count({ where: { deletedAt: null } }),
    prisma.warehouseReceipt.groupBy({
      by: ["status"],
