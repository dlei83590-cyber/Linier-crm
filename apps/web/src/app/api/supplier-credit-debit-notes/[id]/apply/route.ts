...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let noteInfo: { code: string; noteType: string; supplierId: string; sourceSupplierInvoiceId: string; adjustmentTotal: string } | null = null;
  let openAmountAfter: string | null = null;
  try {
    ...
    if (!result.ok) {
      ...
      const mapped = codeMap[result.code] ?? { code: ERROR_CODES.INTERNAL_ERROR, msg: '应用失败（事务已回滚）', status: 500 };
    ...
    if (noteInfo) {
      await publishSupplierCnDnEvent({
        ...
        payload: {
          ...
          supplierId: noteInfo.supplierId,
          sourceSupplierInvoiceId: noteInfo.sourceSupplierInvoiceId,
          adjustmentTotal: noteInfo.adjustmentTotal.toString(),
          openAmountAfter: openAmountAfter ?? undefined,
          appliedById: actorId,
    ...
    return ok({ id, status: 'APPLIED', openAmountAfter: openAmountAfter ?? undefined });
  } catch (err) {
