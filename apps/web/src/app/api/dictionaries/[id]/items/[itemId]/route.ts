...
export async function PATCH(
  ...
  const cas = await casUpdate(prisma, 'dictionaryItem', id, version, { ...updates, version: { increment: 1 }, updatedById: user!.id 
});
  ...
  const updated = await prisma.dictionaryItem.findFirst({ where: { id, deletedAt: null } });
