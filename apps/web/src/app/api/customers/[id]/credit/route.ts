...
/** GET /api/customers/:id/credit（客户信用） */
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const user = await authenticate(request);
  const denied = requirePermission(user, "customer-credit:view");
  if (denied) return denied;
  requestLog(request, user?.id, "customer-credit.get");
...
/** POST /api/customers/:id/credit（创建或更新客户信用，upsert；乐观锁 version 可选） */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const user = await authenticate(request);
  const denied = requirePermission(user, "customer-credit:create");
  if (denied) return denied;
  requestLog(request, user?.id, "customer-credit.upsert");
  ...
  const existing = await prisma.customerCredit.findFirst({ where: { customerId: id, deletedAt: null } });
  if (existing && parsed.data.version !== undefined && existing.version !== parsed.data.version) {
    return failConflict(ERROR_CODES.VERSION_CONFLICT, "版本冲突，请刷新后重试");
  }

  const updated = await prisma.customerCredit.upsert({
    where: { customerId: id },
    update: {
  ...
  await writeAuditLog({
    actorId: user?.id,
    action: "customer-credit.upsert",
    entityType: "customer-credit",
    entityId: updated.id,
    meta: { customerId: id, creditLimit: updated.creditLimit, rating: updated.rating },
    ...meta,
  });
