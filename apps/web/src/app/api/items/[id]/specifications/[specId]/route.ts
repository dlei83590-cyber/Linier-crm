...
export async function PATCH(
  ...
  const cas = await casUpdate(prisma, 'itemSpecification', id, version, { ...updates, version: { increment: 1 }, updatedById: user!.id 
});
  ...
  const updated = await prisma.itemSpecification.findFirst({ where: { id, deletedAt: null } });
