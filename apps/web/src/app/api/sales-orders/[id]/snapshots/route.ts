...
/** GET /api/sales-orders/:id/snapshots（快照列表，generatedAt desc；只读，快照全部由系统在固化节点生成） */
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const user = await authenticate(request);
  const denied = requirePermission(user, "sales-order-snapshot:view");
  if (denied) return denied;
  requestLog(request, user?.id, "sales-order-snapshot.list");
  ...
  const snapshots = await prisma.salesOrderSnapshot.findMany({
