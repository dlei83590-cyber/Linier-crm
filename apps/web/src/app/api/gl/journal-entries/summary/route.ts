...
/**
 * GET /api/gl/journal-entries/summary —— 模块页仪表盘 KPI
 *
...
export async function GET(request: NextRequest) {
  ...
  requestLog(request, user?.id, "gl.summary");

  const [total, byStatus] = await Promise.all([
    prisma.glJournalEntry.count({ where: { deletedAt: null } }),
    prisma.glJournalEntry.groupBy({
      by: ["status"],
