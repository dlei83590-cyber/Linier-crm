...
- 状态：**Draft（2026-08-08，Sprint 4E-1 设计阶段）**——待 CTO Review 拍板 4 项 Pending Decisions（见 §10）
- 日期：2026-08-08
...
## 10. CTO Pending Decisions（4 项待拍板）

| # | 问题 | **默认建议** | 影响面 |
| --- | --- | --- | --- |
| ① | AR 是否在 Invoice ISSUED 时自动创建？ | **是**（issue 同事务创建，失败整体回滚） | issue 路由事务扩展 + 双事件 |
| ② | OVERDUE 是数据库状态还是 effectiveStatus 惰性投影？ | **惰性投影**（不新增 Scheduler） | Schema 双字段 status/effectiveStatus |
| ③ | write-off 直接作用 AR 还是独立实体？ | **独立动作/实体**（4E-2） | 本阶段只留 writeOffAmount |
| ④ | CN/DN 直接调整 AR 还是先生成调整事实？ | **先生成调整事实**（4E-3）再聚合 adjustedAmount | 本阶段只留 adjustedAmount |

## 影响

- Sprint 4E-1 Schema（0018_accounts_receivable_foundation，拍板后实现阶段创建）：**+2 枚举 / +3 表**，仅新增不改既有；不新增 Invoice 列（4D 已有 paidAmount/balanceAmount 投影）。
