...
- **生效边界**：本次交付为「可查、可选、可存、可审计」的权限分配；**运行时鉴权已由 ADR-0057 P2（2026-09-20）切换为 DB 权限集权威**（`authenticate` 解析有效权限集 → `requirePermission` 判定，fail-closed、不回退静态表；内置角色由 seed 回填保持等价）。前端 UI 可见性迁移（`can(code)`，实测 231 处）**已于 ADR-0057 P3 完成**（2026-09-20）。与之相关的 **ADR-0057「动态 RBAC」已 Accepted 并完成 P1/P2/P3**（2026-09-20）：seed 回填内置角色关联、`authenticate` 解析有效权限集、`requirePermission`（666 处引用）改用权限码、前端 231 处判定迁移到 `can()`；仅 **P4 运行时验收**（各角色 200/403 矩阵）待部署环境人工执行。