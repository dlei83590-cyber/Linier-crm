...
- **新建采购 500（根因）**：单号回收（回退整链/删除后 nextNo 回退）后，新建取号命中**软删记录仍占用的编号**（code @unique 约束）→ P2002 未处理 → 500
- 采购链 5 个取号函数（采购申请/采购订单/收货/入库/退货）增加**取号后占用校验**（对齐 #192 报价单先例）：取号后 findUnique 检查编号占用（含软删），占用则 nextNo 递增跳过——新建不再 500

### 边界

- 回收单号语义保留（nextNo 回退）；已被软删占用的编号自动跳过（不误覆盖历史）

---
## [Unreleased] - 全链条一键回退 + 全程回收单号（用户指令 2026-08-21）

### 新增

- **一键回退整链**（仓库收货列表/详情，退货成功后显示「回退整链」）：单事务依次——删除退货单（RETURNED 可删）→ 反质检（删质检）→ 反收货（回滚 PO 履约投影）→ 删除收货单 → 删除采购订单（无其他下链）→ 回退并删除采购申请（CONVERTED→APPROVED→删），退回起点
- **全程回收单号**：退货/收货/入库/采购订单/采购申请删除时若为序列最后一张，DocumentSequence.nextNo 回退一位（CAS 防并发误回退），下次新建复用单号；退货 RETURNED 可删、采购申请 APPROVED（无 PO）可删

### 边界

- GRIR/库存/财务历史事实保留（回退仅软删业务单据并回收单号）；PO 有其他收货/退货下链时停在当前步（不误删）

---
## [Unreleased] - 退货列表操作区 + 全退/部分退 + 检验单防御修复（用户指令 2026-08-21 反馈）

### 修复

- **反收货报错修复**：一键退货时"关联 1 张检验单，禁止反收货"——入库行已全部 RETURNED 退货（错误入库反操作）时忽略检验单防御（货已退回，检验记录仅历史）；未入库/未全退保持防御
- **计算逻辑按实际关联**：列表每项返回可退余额（POSTED 行 quantity − 已 RETURNED 退货），退货按钮与数量校验均基于实际可退

### 新增

- **仓库收货列表操作区「退货」按钮**（POSTED 且有可退余额时显示）：点击拉取入库行明细，默认全退、**数量可改（部分退）**，校验 0 < 数量 ≤ 可退余额；一键完成退货+反收货

### 边界

- 详情页一键退货保持全退；列表页支持全退/部分退

---
## [Unreleased] - 重做采购退货：集成在仓库收货中一键退货+反收货（用户指令 2026-08-21）

### 新增

- **仓库收货（WHR）详情页「退货（一键）」**：已过账（POSTED）且有可退余额的入库单可直接一键退货——
  - 创建退货单（自动带全部可退行，数量=可退余额）+ 完成退货（GRIR REVERSAL + PO 履约 reopen）
  - **反收货**：收货单全部入库行已退货后自动回滚履约投影（收货单回 DRAFT、入库单可删除）——退货与反收货整合为一键完成
  - 处置方式可选：补货（重开 PO 待交）/ 仅退款
- **服务端放宽**：`unreceive` 允许"入库行已全部 RETURNED 退货"时反收货；入库单 DELETE 允许"已过账但全部行已退货"时删除（GRIR 已冲销）
- WHR 详情返回来源采购订单（purchaseReceipt.purchaseOrder）供一键退货使用

### 边界

- 独立"采购退货"模块保留（多来源/部分退货场景）；一键退货覆盖整单可退行
...
- **退货新建/编辑页字段优化**：行内"来源 ID 手填文本框"改为**两级下拉**——选择来源类型（收货行/入库行/质检）→ 选择来源单据（按单拉取）→ 自动拉取该单据可退行供选择（显示行号/物料/数量）
- **按单拉取退货信息**：收货单列表按当前采购订单过滤（purchaseOrderId）；入库单/质检全量；选择单据后拉取详情行（收货行/入库行/质检结论）
- **编辑页回显**：已保存退货行自动反查来源单据（收货单/入库单 id、质检 id）并回填，无需重新手填

### 边界

- 后端来源校验不变（三选一 exactly-one / 来源属于该 PO / POSTED 状态 / 可退余额 SSOT）；无 schema/API 变更
