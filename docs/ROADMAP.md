...
| Sprint 4 | Sales（销售） | 🔄 | 4A Quotation Foundation ✅（PR #12）；4B Sales Order Foundation ✅（PR #13）；4C Delivery Foundation ✅（PR #14）；4D Invoice Foundation ✅（PR #15 已合并）；4E-1 Accounts Receivable Ready for Final Review（PR #16） |
...
## 6. Sprint 4：Sales（销售）🔄（4A Quotation ✅，4B Sales Order ✅，4C Delivery ✅，4D Invoice ✅，4E AR/Payment 下一步）
...
| Accounts Receivable | 应收（余额事实源，Invoice 1:1，账龄/逾期惰性投影） | 🔄 4E-1 Ready for Final Review（PR #16，2026-08-08；CTO Review 97/100 APPROVED WITH CHANGES 已落实；余额唯一口径 + OVERDUE 惰性投影 + agingBucket 不存库 + Snapshot snapshotSource + Invoice 删除保护 + AR 不审批） |
