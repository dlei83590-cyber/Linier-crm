...
| eventType                      | 触发时机                                                                                                                                 | 载荷示例                                                                                                                                             | 实现状态                                |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `PurchaseRequisitionCreated`   | 创建 PR（DRAFT；PR-2026-xxxx 创建即取号；**Header/Line 无金额事实**）                                                                    | `{ requisitionId, requisitionCode, requesterId, departmentId, createdBy }`                                                                           | ✅ 已实现（Sprint 5A Phase 3 PR API）   |
| `PurchaseRequisitionUpdated`   | PATCH 更新 DRAFT 申请（仅非金额字段 needDate/remark + 行整体替换；变更产生 Revision；**PR 无金额事实，不触发重审**）                     | `{ requisitionId, requisitionCode, requesterId, departmentId, changeReason, updatedBy, updatedAt }`                                                  | ✅ 已实现（Sprint 5A Phase 3 PR API）   |
| `PurchaseRequisitionSubmitted` | 提交审批（命中策略触发 Workflow）                                                                                                        | `{ requisitionId, workflowInstanceId, submittedBy, submittedAt }`                                                                                    | ✅ 已实现（Sprint 5A Phase 3 PR API）   |
| `PurchaseRequisitionApproved`  | 审批通过（Workflow 回调，投影回写；**审批不创建 PO**）                                                                                   | `{ requisitionId, workflowInstanceId, approvedBy, approvedAt }`                                                                                      | ✅ 已实现（Sprint 5A Phase 3 PR API）   |
| `PurchaseRequisitionRejected`  | 审批驳回（→ DRAFT 重提）                                                                                                                 | `{ requisitionId, workflowInstanceId, rejectedBy, rejectedAt, reason }`                                                                              | ✅ 已实现（Sprint 5A Phase 3 PR API）   |
| `PurchaseRequisitionConverted` | PR → PO 转单（PR status=CONVERTED；POST /api/purchase-requisitions/{id}/convert）                                                        | `{ requisitionId, purchaseOrderId, purchaseOrderCode, convertedBy, convertedAt }`                                                                    | ✅ 已实现（Sprint 5A Phase 4A Convert） |
| `PurchaseOrderCreated`         | 创建 PO（DRAFT；PO-2026-xxxx 创建即取号；Direct Purchase + PR Convert 双入口）                                                           | `{ purchaseOrderId, purchaseOrderCode, sourceType, supplierId, requisitionId, currency, totalAmount, createdBy }`                                    | ✅ 已实现（Sprint 5A Phase 4A）         |
| `PurchaseOrderUpdated`         | PATCH 更新 DRAFT 订单（仅非金额字段 + 行整体替换；变更产生 Revision；金额服务端重算）                                                    | `{ purchaseOrderId, purchaseOrderCode, changeReason, updatedBy, updatedAt }`                                                                         | ✅ 已实现（Sprint 5A Phase 4A）         |
| `PurchaseOrderSubmitted`       | 提交审批（POST /api/purchase-orders/{id}/submit；DRAFT→SUBMITTED；命中 PURCHASE_ORDER 策略则建/重启 Workflow，无策略直接 APPROVED 投影） | `{ purchaseOrderId, purchaseOrderCode, sourceType, supplierId, requisitionId, workflowInstanceId, currency, totalAmount, submittedBy, submittedAt }` | ✅ 已实现（Sprint 5A Phase 4B Submit）  |
| `PurchaseOrderApproved`        | 审批通过（**内部批准投影；APPROVED ≠ CONFIRMED**——CTO 拍板调整③；Workflow COMPLETED 回调回写，或无策略 Submit 直接投影）                 | `{ purchaseOrderId, workflowInstanceId, approvedBy, approvedAt }`                                                                                    | ✅ 已实现（Sprint 5A Phase 4B）         |
| `PurchaseOrderConfirmed`       | **确认正式下单（POST /api/purchase-orders/{id}/confirm；APPROVED → CONFIRMED；只有 Confirmed PO 才是 5B GR 来源）**                      | `{ purchaseOrderId, purchaseOrderCode, sourceType, supplierId, requisitionId, currency, totalAmount, confirmedBy, confirmedAt }`                     | ✅ 已实现（Sprint 5A Phase 4B Confirm） |
| `PurchaseOrderRejected`        | 审批驳回（→ DRAFT 重提；Workflow REJECTED 回调回写）                                                                                     | `{ purchaseOrderId, workflowInstanceId, rejectedBy, rejectedAt, reason }`                                                                            | ✅ 已实现（Sprint 5A Phase 4B）         |
| `PurchaseOrderCancelled`       | 取消（POST /api/purchase-orders/{id}/cancel；DRAFT/APPROVED 可取消；SUBMITTED 409 走 Withdraw→DRAFT→Cancel；CONFIRMED+ 409 禁止）        | `{ purchaseOrderId, purchaseOrderCode, sourceType, supplierId, requisitionId, currency, totalAmount, cancelledBy, cancelledAt }`                     | ✅ 已实现（Sprint 5A Phase 4B Cancel）  |
...
#### 2.3.9 采购收货领域（Sprint 5B 注册，先注册后开发）

> 来源：Sprint5B_China_ERP_Process_Field_Gate.md / ADR-0024（Approved with Changes，CTO Gate Re-review 96/100 2026-08-09）；**事实链 `PO CONFIRMED → PurchaseReceipt（到货/收货现场事实）→ Inspection（QC 唯一事实源）→ WarehouseReceipt（采购入库事实）→ 6A InventoryMovement(IN)`**；**P10 Final：用业务动作事件，不以 Draft Created 作为完成事实事件**；事件名与状态机一致（`WarehouseReceiptPosted` 与 POSTED 状态一致，不使用 `WarehouseReceiptStocked`）；普通收货/退货不走审批（P1b），超收/特殊退货走 Workflow；超收 System Default = 0%（P2）；RECEIVED PO 禁普通新增收货（D9）；只有 Posted 才触发 InventoryMovement(IN)（D10）；直送（DIRECT_PROJECT）不入库、无 InventoryMovement(IN)（P4）；退货=独立 PurchaseReturn（P5）。

| eventType                        | 触发时机                                                                                                            | 载荷示例                                                                                                                   | 实现状态                                                       |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `PurchaseReceiptReceived`        | 收货完成（PurchaseReceipt 置 RECEIVED；普通收货不走审批 P1b）                                                       | `{ purchaseReceiptId, purchaseReceiptCode, purchaseOrderId, supplierId, warehouseId, receivedById, receivedAt }`           | ✅ PurchaseReceipt API（Sprint 5B，CTO 97/100 APPROVED #7045） |
| `InspectionCompleted`            | 质检结论落定（Inspection result=QUALIFIED/PARTIAL/REJECTED；免检=SKIP+QUALIFIED，不绕过 Inspection）                | `{ inspectionId, purchaseReceiptLineId, inspectionMode, result, qualifiedQty, rejectedQty, inspectedById, inspectedAt }`   | ✅ Inspection API（Sprint 5B，CTO 98/100 FINAL APPROVED #7135）        |
| `WarehouseReceiptPosted`         | 入库过账（WarehouseReceipt status DRAFT→POSTED；**Created ≠ Posted，只有 Posted 才触发 6A InventoryMovement(IN)**） | `{ warehouseReceiptId, warehouseReceiptCode, purchaseReceiptId, warehouseId, locationId, postedById, postedAt }`           | ✅ WarehouseReceipt API（Sprint 5B，CTO 98/100 FINAL APPROVED #7135） |
| `PurchaseReturned`               | 退货完成（PurchaseReturn 置 RETURNED；普通退货不审批，特殊退货走 Workflow）                                         | `{ purchaseReturnId, purchaseReturnCode, purchaseOrderId, supplierId, returnType, lines:[{lineId,sourceRefType,sourceId,quantity,disposition}], hasReplacementRequired, hasCreditOnly, returnedById, returnedAt }` | ✅ PurchaseReturn API（Sprint 5B，CTO Re-review 87/100 REQUEST CHANGES #7267） |
| `PurchaseOrderPartiallyReceived` | PO 收货投影聚合（PO status → PARTIALLY_RECEIVED，5B GR 聚合回写）                                                   | `{ purchaseOrderId, purchaseOrderCode, supplierId, receivedQty, remainingReceiveQty }`                                     | ⏳ Schema/Migration 0023（Sprint 5B）                          |
| `PurchaseOrderReceived`          | PO 全部收货（PO status → RECEIVED）                                                                                 | `{ purchaseOrderId, purchaseOrderCode, supplierId, receivedQty }`                                                          | ⏳ Schema/Migration 0023（Sprint 5B）                          |

> 注：`PurchaseOrderPartiallyReceived/Received` 为 PO 收货聚合投影事件（ADR-0023 已预留），5B 实现时联动发布；`GoodsReceived` / `SupplierInvoiceCreated` 属 5C 注册。
...
| 日期       | 版本  | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-10 | v1.22 | CTO PurchaseReturn Re-review（87/100 REQUEST CHANGES #7267）落实：**来源可退余额修正**（RECEIPT_LINE=`rejectedOnReceiptQty` / INSPECTION=`rejectedQty` / WAREHOUSE_RECEIPT_LINE=`quantity`；Create 预检查与 Return Gate 同步同源，防分叉）；**REPLACE_REQUIRED 真正 reopen PO 履约**（Return 事务内锁 PO Line：INSPECTION/WAREHOUSE 来源 `receivedQty-=returnQty`、`remainingReceiveQty` 重开待交；RECEIPT_LINE 收货时未计入 receivedQty 不重复 reopen；PO 原状态 RECEIVED + 有效 reopen → 重聚回 PARTIALLY_RECEIVED）；**PurchaseReturned 载荷 line-level disposition**（`lines[]` + `hasReplacementRequired/hasCreditOnly`，弃单值第一行 disposition）；**5B 仍禁写 Stock/InventoryMovement**（6A 唯一事实源）；PO 投影 2 事件仍 ⏳ |
| 2026-08-10 | v1.21 | Sprint 5B PurchaseReturn API 落实：`PurchaseReturned` ⏳→✅（**只有 return 事务成功后发布**；载荷含退货单/来源 PO/供应商/类型/处置/操作人/时间，**不含库存余额**——6A 唯一事实源）；**退货硬约束（CTO #7219 第一版锁死）**：必须有真实来源（RECEIPT_LINE/INSPECTION=未入库退货不碰库存；WAREHOUSE_RECEIPT_LINE=必须来自 POSTED 入库事实但**不写 InventoryMovement(OUT)**）、quantity ≤ 来源可退余额、**Return Gate 锁真实来源并锁内重算累计 RETURNED（防多单并发超退）**、disposition（REPLACE_REQUIRED 重开 PO 履约剩余 / CREDIT_ONLY 不自动重开）、RETURN 事务锁 + CAS/幂等（ALREADY_RETURNED→409）+ Audit + 事件后发；**5B 仍禁写 Stock/InventoryMovement**（6A 唯一事实源）；财务冲减/红字发票/AP 属 5C；PO 投影 2 事件仍 ⏳ |
| 2026-08-10 | v1.20 | Sprint 5B WarehouseReceipt API 落实：`WarehouseReceiptPosted` ⏳→✅（**只有 POST 事务成功后发布**；D10：Created ≠ Posted，只有 Posted 才触发 6A InventoryMovement(IN)；载荷含入库单/来源收货/仓库库位/操作人/时间，**不含库存余额**——6A 唯一事实源）；**入库硬约束（CTO #7135 第一版锁死）**：来源 Inspection 必须已完成且 qualifiedQty>0、累计入库 ≤ 可入库余额（POST 时含本单行，防并发超入）、Warehouse-Location 组合 FK 同属、DIRECT_PROJECT 禁入库（P4）、POST 事务锁 + CAS/幂等（ALREADY_POSTED→409）、DRAFT ≠ POSTED、DRAFT 变更不发事件；**5B 仍禁写 Stock/InventoryMovement**；PurchaseReturned / PO 投影 2 事件仍 ⏳（PurchaseReturn API 后续） |
| 2026-08-09 | v1.19 | Sprint 5B Inspection API 落实：`InspectionCompleted` ⏳→✅（**只有 complete 事务成功后发布**；载荷含检验行/模式/结论/数量/操作人/时间，对齐 2.3.9）；`PurchaseReceiptReceived` ⏳→✅（PurchaseReceipt API 已实现，CTO Final Re-check 97/100 APPROVED #7045）；**质检语义锁定**：inspectableQty = quantity - rejectedOnReceiptQty（最大可检数不再次包含现场拒收）、一次 Inspection 即最终结果 → qualifiedQty + rejectedQty = inspectableQty、SKIP+QUALIFIED 免检不绕过 Inspection、result 服务端推导（客户端不得传）、Inspection 禁写 Stock/InventoryMovement（6A 唯一事实源）；WarehouseReceiptPosted / PurchaseReturned / PO 投影 2 事件仍 ⏳（WarehouseReceipt/PurchaseReturn API 后续）                                                                                             |
| 2026-08-09 | v1.18 | Sprint 5B 注册采购收货领域事件 **6 个**（PurchaseReceiptReceived / InspectionCompleted / WarehouseReceiptPosted / PurchaseReturned + PurchaseOrderPartiallyReceived / PurchaseOrderReceived，统一载荷含单据 id/code/来源/数量/操作人；**P10 Final：业务动作事件，命名与状态机一致（WarehouseReceiptPosted，不使用 WarehouseReceiptStocked）**；普通收货/退货不走审批、超收 System Default=0%、RECEIVED 禁普通新增收货（D9）、只有 Posted 才触发 6A InventoryMovement(IN)（D10）、直送不入库（P4）、退货=独立 PurchaseReturn（P5）；见 2.3.9，ADR-0024）；GR/Supplier Invoice 事件 5C 注册                                                                                                                                                                                              |
| 2026-08-09 | v1.17 | Sprint 5A Phase 4B PO 审批链落实：`PurchaseOrderSubmitted/Approved/Confirmed/Rejected/Cancelled` 5 事件 ⏳→✅（Submit / Workflow 回调 / Confirm / Cancel 路由）；**PurchaseOrderSnapshot 唯一约束修复** `[purchaseOrderId, snapshotType]` → `[purchaseOrderId, snapshotType, revisionNo]`（多轮审批不冲突，Migration 0022）；**PO Header +purchaserId/departmentId**（采购员/采购部门，Direct PO 无 PR 可溯，Migration 0022）；**红线锁死 APPROVED≠CONFIRMED**（Workflow 只回写 APPROVED，唯一 Confirm 路由置 CONFIRMED，只有 Confirmed PO 是 5B GR 来源）；Cancel 规则：DRAFT/APPROVED 可取消，SUBMITTED 409 走 Withdraw→DRAFT→Cancel，CONFIRMED+ 409 禁止；审批复用 Workflow 单实例多轮重提（对齐 SalesOrder/Invoice/WriteOff/PR 模式）                                              |
| 2026-08-09 | v1.16 | Sprint 5A Phase 4A PO API 落实：`PurchaseRequisitionConverted`（Convert 路由）与 `PurchaseOrderCreated`（Direct + Convert 双入口）⏳→✅，新增注册 `PurchaseOrderUpdated`（PATCH 更新 DRAFT 订单，仅非金额字段 + 行整体替换，变更产生 Revision，金额服务端重算）✅；`PurchaseOrderSubmitted/Approved/Confirmed/Rejected/Cancelled` 仍 ⏳（Phase 4B：Submit/Approval/Confirm/Cancel，**APPROVED≠CONFIRMED**，Confirm 生成 CONFIRMED Snapshot）                                                                                                                                                                                                                                                                                                                                           |
| 2026-08-09 | v1.15 | Sprint 5A Phase 3 PR API 落实：`PurchaseRequisitionCreated/Submitted/Approved/Rejected` 4 事件 ⏳→✅（实现于 Phase 3 PR API）；新增注册 `PurchaseRequisitionUpdated`（PATCH 更新 DRAFT 申请，仅非金额字段 needDate/remark + 行整体替换，变更产生 Revision，**PR 无金额事实不触发重审**）✅；`PurchaseRequisitionConverted` 仍 ⏳（属 PO 阶段显式动作）；PO 事件仍 ⏳（PO API 冻结）                                                                                                                                                                                                                                                                                                                                                                                                    |
