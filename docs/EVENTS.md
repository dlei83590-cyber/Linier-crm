...
| eventType | 触发时机 | 载荷示例 | 实现状态 |
| --- | --- | --- | --- |
| `PurchaseReceiptReceived` | 收货完成（PurchaseReceipt 置 RECEIVED；普通收货不走审批 P1b） | `{ purchaseReceiptId, purchaseReceiptCode, purchaseOrderId, supplierId, warehouseId, receivedById, receivedAt }` | ⏳ Schema/Migration 0023（Sprint 5B） |
| `InspectionCompleted` | 质检结论落定（Inspection result=QUALIFIED/PARTIAL/REJECTED；免检=SKIP+QUALIFIED，不绕过 Inspection） | `{ inspectionId, purchaseReceiptLineId, inspectionMode, result, qualifiedQty, rejectedQty, inspectedById, inspectedAt }` | ⏳ Schema/Migration 0023（Sprint 5B） |
