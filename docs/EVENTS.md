...
| eventType | 触发时机 | 载荷示例 | 实现状态 |
| --- | --- | --- | --- |
| `ReceiptCreated` | 创建收款单（只记录金额，不核销——拍板①） | `{ receiptId, receiptCode, customerId, currency, amount, receiptDate, paymentMethod }` | ⏳ 注册待实现（Sprint 4E-2） |
...
| `ReceiptFullyAllocated` | 全部核销完成（unallocatedAmount = 0） | `{ receiptId, receiptCode, allocatedAmount, unallocatedAmount, fullyAllocatedAt }` | ⏳ 注册待实现（Sprint 4E-2） |
| `ReceiptAllocationReversed` | Allocation Reversal（解除核销，回退 AR/Invoice/Receipt 投影；CTO Design Review 新锁定边界） | `{ receiptAllocationId, receiptId, accountsReceivableId, reversedAmount, paidAmount, balanceAmount, reversedBy, reversedAt, reason }` | ⏳ 注册待实现（Sprint 4E-2） |
| `ReceiptVoided` | 作废（仅未核销可 VOID；已核销先 Reversal 再处理，CN 不承担收款冲销） | `{ receiptId, receiptCode, voidedBy, voidedAt, reason }` | ⏳ 注册待实现（Sprint 4E-2） |
...
| 日期 | 版本 | 说明 |
| --- | --- | --- |
| 2026-08-08 | v1.10 | Sprint 4E-2 注册收款/核销领域事件 11 个（ReceiptCreated/ReceiptUpdated/ReceiptAllocated/ReceiptFullyAllocated/ReceiptAllocationReversed/ReceiptVoided + WriteOffCreated/WriteOffSubmitted/WriteOffApproved/WriteOffRejected/WriteOffApplied，统一载荷含 receiptId/writeOffId；Receipt=收款事实源，Payment 不单独建表；**创建与核销分离**；核销 M:N + 锁 AR（ID ASC+FOR UPDATE）+ 防超核销 409；**同客户同币种**（409，禁跨币种）；**Allocation/Receipt Reversal 属 4E-2、CN 不承担收款冲销**；WriteOff 独立事实不 PATCH AR；Receipt 不审批、WriteOff 条件触发 Workflow；见 2.3.6，CTO Design Review 97/100 落实）；AR PartiallyPaid/Paid/WrittenOff/Closed 与 Invoice 投影事件在 4E-2 实现时联动发布 |
