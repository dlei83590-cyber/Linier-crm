...
| `InspectionCompleted`            | 质检结论落定（Inspection result=QUALIFIED/PARTIAL/REJECTED；免检=SKIP+QUALIFIED，不绕过 Inspection）                | `{ inspectionId, purchaseReceiptLineId, inspectionMode, result, qualifiedQty, rejectedQty, inspectedById, inspectedAt }`   | ✅ Inspection API（Sprint 5B）                                 |
| `WarehouseReceiptPosted`         | 入库过账（WarehouseReceipt status DRAFT→POSTED；**Created ≠ Posted，只有 Posted 才触发 6A InventoryMovement(IN)**） | `{ warehouseReceiptId, warehouseReceiptCode, purchaseReceiptId, warehouseId, locationId, postedById, postedAt }`           | ⏳ Schema/Migration 0023（Sprint 5B）                          |
