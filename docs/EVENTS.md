...
| `PurchaseReturned`               | 退货完成（PurchaseReturn 置 RETURNED；普通退货不审批，特殊退货走 Workflow）                                         | `{ purchaseReturnId, purchaseReturnCode, purchaseOrderId, supplierId, returnType, disposition, returnedById, returnedAt }` | ✅ PurchaseReturn API（Sprint 5B，CTO WarehouseReceipt 98/100 FINAL #7219） |
