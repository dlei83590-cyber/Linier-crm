...
| 模块 | 后端契约（FINAL） | 关键动作 | 页面形态 |
| --- | --- | --- | --- |
| Stock Projection | 6A `api/stock-projections`（**Read Model Gate FINAL，2026-08-18**）✅ | 五维余额展示（item/warehouse/location/batch/serial） | ✅ 只读列表（`stock-projection:view`；余额 SSOT，前端不计算） |
| Inventory Movement Ledger | 6A `api/inventory-movements`（**Read Model Gate FINAL，2026-08-18**）✅ | 流水追溯（Trace/Audit，**非余额 API**） | ✅ 只读列表 + 详情（`inventory-movement:view`） |
...
- ✅ **CTO #8845 Contract Blocking 解除（Inventory Read Model Gate FINAL，2026-08-18）**：`GET /api/stock-projections` / `GET /api/inventory-movements` 已发布 FINAL 只读端点（权限 `stock-projection:view` / `inventory-movement:view`）——Stock Projection / Inventory Movement Ledger 两页已接线；前端**仍不得多 API 拼装余额、不得 SUM Movement 充当 StockProjection**（§14/§16）。
