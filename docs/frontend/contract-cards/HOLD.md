...
- 能力：contract NONE / ui NONE（双层模型，CTO F2-1 Review）
...
- 能力：contract NONE / ui NONE（双层模型，CTO F2-1 Review）；无 API 路由（permission 已 seed）
- Gap：待后端契约
...
- 能力：contract NONE / ui NONE（双层模型，CTO F2-1 Review）；无 API 路由
- Gap：待后端契约；F2-4 中作为 Project Detail Workspace Tabs 的子资源规划
  （不开放独立 Sidebar 入口）

## 库存展望 stock-projection / 库存流水 inventory-ledger

- 能力：contract NONE / ui NONE（双层模型，CTO F2-1 Review）；无 FINAL Read API（`/api/inventory-ledger/consume` 为消耗端，非列表读模型）
...
- 能力：contract NONE / ui NONE（双层模型，CTO F2-1 Review）；无 API 路由、无独立 permission（禁止复用销售侧 CREDIT_DEBIT_NOTE_READ）
...
- 能力：contract NONE / ui NONE（双层模型，CTO F2-1 Review）；无 API 路由（指标未定义）
