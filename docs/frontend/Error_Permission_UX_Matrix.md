...
| Stock Projection | `/inventory/stock-projection` | `stock-projection:view`（✅ Read Model Gate FINAL，2026-08-18） | `<Forbidden />` |
| Inventory Movement Ledger | `/inventory/ledger` | `inventory-movement:view`（✅ Read Model Gate FINAL，2026-08-18） | `<Forbidden />` |
...
> ✅ **CTO #8845 Contract Blocking 解除（Inventory Read Model Gate FINAL，2026-08-18）**：`stock-projection:view` / `inventory-movement:view` 已是正式生产权限事实（shared PERMISSION_MODULES + seed 同步注册，ADR-0028）；Stock Projection / Inventory Ledger 两页已接线 FINAL 只读 API；前端仍不得多 API 拼装余额、不得 SUM Movement 充当 StockProjection（§14/§16）。
