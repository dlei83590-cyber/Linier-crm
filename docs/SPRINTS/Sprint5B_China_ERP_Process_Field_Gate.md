...
| 事实属性 | 供应商实际送达的数量（**可小于、等于或大于 PO 订购数量**——超收是独立问题，见 §7） |
| 与库存关系 | **不直接产生库存增加**；只更新 PO Line 收货投影（receivedQty / remainingReceiveQty 的"已到货"口径） |
| 关键约束 | 来源必须是 CONFIRMED（或已部分收货）PO；无 Direct GR（对齐无 Direct Delivery 锁定项） |
...
| 触发方 | 质检员（QC） |
| 事实属性 | 合格数量、不合格数量、拒收数量、检验结论、检验单号（如需） |
| 与库存关系 | **只有"合格可入库"数量才允许进入仓库**；不合格/拒收数量走退货或供应商处理 |
| 模式选择 | 免检（默认合格）/ 抽检 / 全检——**Pending Decision P3** |
...
| 关键约束 | 入库数量 ≤ 验收合格数量 ≤ 到货数量（逐层 ceiling）；超收上限见 §7 |
...
| 触发方 | 采购/项目侧标记 + 收货确认 |
| 事实链 | `PO CONFIRMED → Receipt / Inspection → Direct Delivery → 不进入 Warehouse Stock` |
| 与库存关系 | 不产生 InventoryMovement(IN)；消费/领用由项目侧另行记录（6D 或项目成本） |
| 关键约束 | 必须显式标记直送（不能"忘了入库"）；直送判定与字段见 **Pending Decision P4** |
...
| 事实链 | `PurchaseReturn` = **独立业务事实**（不是负数 GR）——CTO #6680 倾向 |
...
### 3.2 方案 B：拆两层（CTO 倾向，推荐）

| 层 | 模型（草案名，非 Schema） | 职责 |
| --- | --- | --- |
| 收货层 | `PurchaseReceipt`（到货/收货事实） | 记录供应商实际送达；更新 PO 收货投影；可带收货备注/收货单号 |
| 入库层 | `WarehouseReceipt`（采购入库事实） | 记录"合格数量正式入库到仓库/库位"；携带批次/序列号/效期；**是驱动 InventoryMovement(IN) 的业务事实** |
...
| ceiling | 累计到货数量 ≤ PO Line 订购数量 + 超收容差（§7） |
| 投影回写 | 每次 PurchaseReceipt 后更新 PO Line `receivedQty`（累计到货口径）与 `remainingReceiveQty` |
| 全部收完 | 累计到货 ≥ 订购数量（且无退货挂起）→ PO status → `RECEIVED`；否则 `PARTIALLY_RECEIVED` |
...
| 规则 | 草案 |
| --- | --- |
| 允许超收？ | 默认允许**小容差**（建议 5%，**Pending Decision P2**）；超容差需审批 |
| 容差计算 | 单 Line 维度：`累计到货 ≤ quantity × (1 + tolerance%)`；超出 → 拒绝或转审批 |
| 超容差审批 | 走 Workflow（module=PURCHASE_RECEIPT 或复用采购审批）→ 审批通过才允许录入 |
...
| 规则 | 草案 |
| --- | --- |
| 模式 | 免检 / 抽检 / 全检（按 Item 或供应商配置，**Pending Decision P3**） |
| 结论 | 合格 / 部分合格 / 拒收 / 待检（待检 = 未决，不产生入库资格） |
| 数量关系 | 合格入库数量 ≤ 合格数量 ≤ 到货数量（逐层 ceiling） |
...
| 规则 | 草案 |
| --- | --- |
| 第一版 | 单仓 + 可选库位（**Pending Decision P8**：是否多仓 / 库位是否必填） |
| 模型来源 | 复用/新建 Warehouse 主数据（6A 会建 Warehouse/Location——5B 只引用不新建，或 5B 先行最小 Warehouse 主档，待 6A 扩展） |
...
| 规则 | 草案 |
| --- | --- |
| 采集时机 | 推荐在 **WarehouseReceipt（入库层）** 采集，而非收货层——只有合格入库才需要批次信息（**Pending Decision P6**） |
...
| 项 | 草案 |
| --- | --- |
| 独立事实 | `PurchaseReturn` 独立模型（Header + Lines），不做负数 GR |
...
## 11. 事件注册（5B，先注册后开发，对齐 EVENTS.md 纪律）

| eventType | 触发 | 注册时机 |
| --- | --- | --- |
| `PurchaseReceiptCreated` | 到货/收货录入 | 5B 实现时 |
| `PurchaseReceiptRejected`（如需） | 收货撤销 | 5B 实现时 |
| `InspectionCompleted`（如需） | 质检结论落定 | 5B 实现时 |
| `WarehouseReceiptCreated` | 入库事实生成 | 5B 实现时 |
| `PurchaseReturnCreated` | 退货事实生成 | 5B 实现时 |
| `PurchaseOrderPartiallyReceived` / `PurchaseOrderReceived` | PO 收货投影聚合（ADR-0023 已预留） | 5B 实现时 |
| `GoodsReceived` | 5C 注册（EVENTS.md 已注明） | 5C |

> 具体事件命名与载荷待 CTO Design Review 确认（**Pending Decision P10**）。
