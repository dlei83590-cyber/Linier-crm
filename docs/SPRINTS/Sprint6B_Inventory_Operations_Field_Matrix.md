...
- 版本：v0.4（CTO 6B Schema Review #8112 86/100 REQUEST CHANGES——4 Blocking + 2 Integrity 修正已落实：Adjustment direction 下沉到 Line、sourceStockCountLineId UNIQUE 防双重入账、Conversion 单输入单输出 UNIQUE(conversionHeaderId, lineRole)、Conversion Line 行级换算率 uomToBaseRate/baseQuantity（删 header 单一 conversionRate）、Transfer/Conversion/Adjustment 终态证据 CHECK、InventoryAdjustment.createdById NOT NULL；Schema/Migration 0026 已修正；API/Workflow/Seed/RBAC/共享 LedgerCommand core 抽取/Consumer 改造仍 HOLD）
- 日期：2026-08-11
- 维护者：CIO（JINZA）｜审核：CTO
- 关联：Sprint6B_Inventory_Operations_Architecture_Process_Gate.md / ADR-0026（Approved with Changes）/ Sprint6B_CTO_Pending_Decisions.md / Sprint6A_Inventory_Field_Matrix.md（6A：InventoryMovement / StockProjection / OutboxMessage 已实现）/ EVENTS.md（v1.28）
...
| sourceStockCountId                     | 来源盘点单（可空）           | FK → StockCount                                                                                                                       | Manual 调整无盘点来源                                                                                                                                                    |
...
| **uomToBaseRate**        | 行级换算率 snapshot | Decimal(18,6)                                                                                             | **Blocking ④：业务 UOM → base UOM 换算率**（行级声明，不隐式查表）                                                                                                                                                                                        |
| **baseQuantity**         | canonical 数量      | Decimal(18,4)                                                                                             | **Blocking ④：baseQuantity = quantity × uomToBaseRate**；service EXECUTE 前验证 CONSUME.baseQuantity == PRODUCE.baseQuantity；真正写 InventoryMovement 时 quantity=baseQuantity、uomId=conversion.baseUomId——Ledger 只看 canonical 数量，不再依赖业务 UOM |
