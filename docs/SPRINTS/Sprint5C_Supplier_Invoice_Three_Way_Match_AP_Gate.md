...
- 状态：**设计先行——禁止 Schema / Migration / API**（CTO Gate 批准后才允许）
- 关联：ADR-0027（草案）/ Sprint5C_Field_Matrix.md / Sprint5C_CTO_Pending_Decisions.md / EVENTS.md / ADR-0023（5A Implemented）/ ADR-0024（5B Implemented）/ ADR-0025（6A Implemented）/ ADR-0026（6B Implemented）
...
| 事实属性 | AP 金额（含税）、已付金额、余额（openAmount）、到期日、币种、发票溯源 |
| 与付款关系 | **AP Open Item**（未清项）→ Payment Allocation（付款核销）→ 余额归零 |
...
| 触发方 | 系统（WarehouseReceipt Posted 时自动生成暂估，或月度结账时批量） |
| 与发票关系 | 到票时**冲销暂估**（到票冲暂估），按发票实际金额确认 AP；暂估与实票差异计入差异处置 |
...
| # | 设计维度 | 初步边界 |
| --- | --- | --- |
| 4.1 | **已收未票** | WarehouseReceipt Posted 但无 Supplier Invoice → 暂估应付（GR/IR）候选；需 Pending Decision：自动暂估 vs 月结批量暂估 |
| 4.2 | **暂估应付** | 按 PO 快照单价 × 已入库数量暂估；暂估不产生真实 AP Open Item（是过渡投影，到票冲销） |
| 4.3 | **到票冲暂估** | 发票到达并匹配后：冲销暂估投影 → 按发票实际金额生成 AP Liability；暂估与实票差异走差异处置 |
...
| 4.8 | **进项税** | 税基 = 匹配后的净额；税率快照（对齐 4D Invoice 快照税务模式）；进项税作为 AP 金额组成部分（含税 AP）或分离（不含税 AP + 税）——Pending Decision |
| 4.9 | **发票多次/分批到票** | 一张 PO 可对应多张 Supplier Invoice（部分开票）；累计开票金额 ≤ PO 行可开票余额（锁内重算防超开，对齐 4E Partial Billing + 5B 来源可退余额模式） |
| 4.10 | **Credit Note / Supplier Debit/Credit** | Supplier CN（冲减 AP）/ Supplier DN（增加 AP）；signed adjustment（CN<0/DN>0）+ 累计防超调锁内重算（对齐 4E-3 模式，方向相反） |
| 4.11 | **AP Open Item** | 未清项模型：AP 金额 - 已核销 = openAmount；到期日/账龄（对齐 4E-1 AR aging 模式，方向相反） |
| 4.12 | **Payment Allocation** | 付款单 + M:N 核销 AP Open Item；防超核销（累计 allocation ≤ openAmount）；同供应商同币种（对齐 4E-2 模式，方向相反） |
| 4.13 | **Supplier Invoice 状态机** | 草案：`DRAFT → SUBMITTED → MATCHED → APPROVED → POSTED（AP Liability 生成）/ CANCELLED`；差异路径（VARIANCE → 处置）待 Pending Decision |
...
## 5. 关键设计原则（CTO 拍板方向 + 待确认项）

1. **Supplier Invoice ≠ Sales Invoice 反向版**（CTO #8777 锁死）：独立模型，可借鉴 4E 模式但方向/溯源/状态机不同
2. **3-Way Match 是校验事实，不是冲销事实**：匹配结果 + 差异处置 → 才形成 AP Liability
3. **暂估应付是过渡投影，不是真实债务**：到票冲销；暂估与实票差异进差异处置
4. **发票金额是财务事实，不是成本事实**：Costing（FIFO/移动平均）继续 HOLD
5. **5C 全程不触碰库存**：不写 InventoryMovement / StockProjection（6A SSOT 红线继承）
6. **maker-checker / 审批边界**：发票过账（POSTED）与付款（Payment）高权限强审计；差异审批走 Workflow（Pending Decision）
7. **事件纪律**：业务层事件（SupplierInvoiceCreated / Matched / Posted / Paid 等）注册 EVENTS.md，事务后发布、不含余额（对齐 6B 模式）
...
| `SupplierInvoiceMatched` | 三单匹配完成（MATCHED / VARIANCE 结果） | ⏳ 5C 注册 |
| `SupplierInvoicePosted` | 发票过账（POSTED，AP Liability 生成） | ⏳ 5C 注册 |
| `SupplierInvoiceCancelled` | 发票取消 | ⏳ 5C 注册 |
| `SupplierCreditNoteApplied` / `SupplierDebitNoteApplied` | 供应商 CN/DN 生效 | ⏳ 5C 注册 |
| `ApPaymentAllocated` | 付款核销 AP Open Item | ⏳ 5C 注册 |
| `GoodsReceived` | 5B 已预留（收货完成聚合投影）——本阶段确认是否注册 | ⏳ 待 Pending Decision |
...
## 8. CTO Design Review 后动作

1. P1-P12（Pending Decisions）全部拍板 → 固化进 ADR-0027 / Gate / Field Matrix
2. Schema / Migration 0027（未进 main 前直接修）→ Seed/RBAC → Supplier Invoice API → 3-Way Match → AP → Payment → Workflow/Event/Audit → OpenAPI/QA/Test Cases
3. commit → push → GitHub CI → STOP → CTO 5C Design Review
