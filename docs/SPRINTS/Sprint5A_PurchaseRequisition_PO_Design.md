...
> **状态：Ready for CTO Design Review（本阶段只做 Design / ADR / EVENTS，禁止 Schema / Migration / API）**
...
- 关键字段草案：`id / code / supplierId / requisitionId(溯源，可空=直采) / status / currency / paymentTerm / totalAmount / workflowInstanceId / approvalStatus / approvedAt / approvedById / lines / revisions / snapshots / 审计字段`
- 状态机草案：`DRAFT → SUBMITTED → APPROVED → PARTIALLY_RECEIVED → RECEIVED → COMPLETED`；`DRAFT/SUBMITTED → CANCELLED`（RECEIVED 投影由 5B GR 聚合回写，对齐 SO deliveredQty）
- 审批：条件审批复用 ApprovalPolicy(module=`PURCHASE_ORDER`) + Workflow（不建 Approval 表）
- 行：`PurchaseOrderLine`：`id / purchaseOrderId / lineNo / sourceRequisitionLineId(溯源，可空) / itemId / description / quantity / uomId / unitPrice(快照) / discountRate / taxRate(快照) / lineAmount / taxAmount / totalAmount / 审计字段`；`@@unique([purchaseOrderId, lineNo])`

### 3.3 Revision / Snapshot

- **PO 对齐 Sales Order 模式**：`PurchaseOrderRevision` + `PurchaseOrderSnapshot`（快照类型：CREATED/SUBMITTED/APPROVED/RECEIVED/CANCELLED）
...
### 5.3 PO Convert（PR→PO，草案；Pending 决策②定入口）

```
Lock PR（FOR UPDATE）→ 校验 PR 状态（APPROVED 才能转单）
→ 取号（DocumentSequence PURCHASE_ORDER 原子 increment）
→ 创建 PurchaseOrder（DRAFT）+ Lines（快照复制自 PR Line / 供应商价格来源）
...
## 6. 价格/金额事实来源（Pending 决策③，最重要的拍板项）

| 选项 | 描述 | 优劣 |
| --- | --- | --- |
| A（推荐） | PO 行价来自 **Supplier 价格快照**（partner-prices priceSource=SUPPLIER 或 Supplier 专属价格表），转单/创建时快照复制，**不重算** | 对齐销售侧价格红线；可追溯 |
| B | 允许手工录入单价（PO 创建时填价），快照复制 | 灵活但引入人为定价风险 |
| C | PR 的 suggestedUnitPrice 直接带进 PO | 需求阶段价格不严谨，不推荐 |

- **金额事实链（对齐销售侧）**：Supplier 价格（PartnerPrice）→ PO Line 单价快照 → PO.totalAmount（Σ 行，服务端计算）；**PO 不调 Pricing Engine、不重算**
- **税率**：PO 行 taxRate 快照（对齐 Invoice 快照税务），税档变化不影响已 APPROVED PO（Pending 决策④）
...
## 8. Goods Receipt 边界（5B，本阶段只定义）

- PO Line 预留投影语义：`receivedQty / remainingReceiveQty`（5B GR 聚合回写，对齐 SO deliveredQty/remainingQty 动态计算先例——**本阶段不建列**，5B 实现时按 computeDeliveryAllocation 思路动态算）
...
## 9. 事件注册（EVENTS.md v1.14，先注册后开发，见 2.3.8）
...
| `PurchaseOrderApproved` | 审批通过（= 正式下单语义） | `{ purchaseOrderId, workflowInstanceId, approvedBy, approvedAt }` | ⏳ 注册待实现 |
...
- **Migration 0021**（实现阶段草案）：新增 `PurchaseRequisition`/`PurchaseRequisitionLine`/`PurchaseRequisitionRevision`/`PurchaseOrder`/`PurchaseOrderLine`/`PurchaseOrderRevision`/`PurchaseOrderSnapshot` + 枚举（PurchaseRequisitionStatus/PurchaseOrderStatus/PurchaseOrderSnapshotType）；纯增量不改既有；DocumentType 新增 `PURCHASE_REQUISITION`
...
## 11. CTO Pending Decisions（Design Review 拍板项）

| # | Pending | 选项 |
| --- | --- | --- |
| ① | PR/PO 审批链 | A. PR + PO 双审批（PR APPROVED 才能转 PO，PO 再审批）｜B. PR 免审、PO 必审（推荐：采购金额事实在 PO，审批聚焦 PO）｜C. 条件审批各自独立（ApprovalPolicy 各自 module，命中才审） |
| ② | PO 创建入口 | A. 仅 PR convert（对齐 Quotation→SO 1:1）｜B. PR convert + 直接创建（紧急直采，requisitionId 可空）｜C. 仅直接创建（不强制 PR） |
| ③ | PO 价格事实来源 | A. Supplier 价格快照（partner-prices SUPPLIER）复制，不重算（推荐）｜B. 手工录入快照｜C. PR suggestedUnitPrice 带入 |
| ④ | 税率快照策略 | A. PO 行 taxRate 快照复制，税档变化不影响已 APPROVED PO（推荐，对齐 Invoice）｜B. 实时取当前税档 |
| ⑤ | PR 是否带金额 | A. PR 无金额（纯需求，金额事实在 PO）（推荐）｜B. PR 带预估金额（suggestedUnitPrice × qty） |
| ⑥ | PR Revision/Snapshot | A. PR 仅 Revision（快照延后）｜B. PR 全套 Revision+Snapshot（对齐 PO） |
| ⑦ | PO 修改重审 | A. SUBMITTED 后 PATCH 财务字段触发重审（对齐 Invoice keyFinancialChanged，推荐）｜B. SUBMITTED 后冻结，仅 DRAFT 可改 |
